...
def load_data():
    import os
    df1 = pd.read_csv('transaction_1.csv')
    df2 = pd.read_csv('transaction_2.csv')
    df = pd.concat([df1, df2], ignore_index=True)
    df['Date'] = pd.to_datetime(df['DateTime']).dt.date
    df['Date'] = pd.to_datetime(df['Date'])
    df = df[df['Date'] < '2025-10-01']

    sku_meta    = df[['SKU_ID','SKU','Brand','SKU_Category']].drop_duplicates().reset_index(drop=True)
    ...
    price_lookup = df.groupby('SKU_ID')['NormalPrice'].mean().to_dict()

    return df, sku_meta, matrix_df, elast_lookup, price_lookup, forecast_df
...
def simulate_portfolio_impact(sku_a_id, discount_pct, branch, weeks=4):
    ...
    base = (forecast_df[forecast_df['Branch'] == branch]
            .groupby(['SKU_ID','ForecastWeek'])['BaseForecast']
            .mean().reset_index())
    ...
    for week in range(1, weeks + 1):
        for sku_b_id in skus:
            base_b_val = base[(base['SKU_ID']==sku_b_id) &
                               (base['ForecastWeek']==week)]['BaseForecast']
            base_qty_b = base_b_val.values[0] if len(base_b_val) > 0 else 0
            ...
            if sku_b_id == sku_a_id:
                ...
                base_a_val = base[(base['SKU_ID']==sku_a_id) &
                                   (base['ForecastWeek']==week)]['BaseForecast']
                base_qty_a = base_a_val.values[0] if len(base_a_val) > 0 else 0
